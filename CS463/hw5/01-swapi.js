...
const mapStarships = (input) => {
  ...
  const results = input.map((v) => {
    let str = `${v.name}, manufactured by ${v.manufacturer}`;

    if (v.cost_in_credits !== "unknown") {
      str += ` - cost: ${parseInt(v.cost_in_credits).toLocaleString()} credits`;
    }
...
const filterStarships = (input) => {
  ...
  const results = input.filter((v) => v.passengers < 10 && v.crew > 1);
  return results;
};

const reduceStarships = (input) => {
  // Return the cost to purchase all ships in the input array
  const results = input.reduce((accumulator, current) => {
    if (current.cost_in_credits !== "unknown") {
      return accumulator + parseInt(current.cost_in_credits);
    } else {
      return accumulator;
    }
  }, 0);

  return `The cost of all starships is ${results.toLocaleString()} credits`;
};
